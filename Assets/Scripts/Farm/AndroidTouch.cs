...
public class AndroidTouch : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButton(0)) {
            movedx = Input.mousePosition - oldMousePos;           
            targetPos = new Vector3(Mathf.Clamp(transform.position.x + -movedx.x * dragSmooth * Time.deltaTime, -5, 5),
                transform.position.y, transform.position.z);
            oldMousePos = Input.mousePosition;
        }
        if (Vector3.Distance(transform.position, targetPos) > 0.001f) {
            transform.position = Vector3.MoveTowards(transform.position, targetPos, moveSP * Time.deltaTime);
        }
        return;
